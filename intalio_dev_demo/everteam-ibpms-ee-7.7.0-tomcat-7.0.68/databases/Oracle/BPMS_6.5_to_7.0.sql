...
ALTER TABLE tempo_pa ADD ctm_xml CLOB ;
...
ALTER TABLE tempo_audit ADD prev_var_data BLOB;
ALTER TABLE tempo_audit ADD new_var_data BLOB;
...
CREATE TABLE module_action_access(
	id NUMBER NOT NULL,
	module_action_id NUMBER NOT NULL,
...
ALTER TABLE BPEL_ACTIVITY_RECOVERY MODIFY REASON VARCHAR2(1000);

CREATE INDEX IDX_EVENT_PID ON BPEL_EVENT(PID);
CREATE INDEX IDX_EVENT_SID ON BPEL_EVENT(SID);

DROP INDEX IDX_PLINK_VAL_MODELID;
CREATE INDEX IDX_PLINK_VAL_MODELID ON BPEL_PLINK_VAL (MODELID);
CREATE INDEX IDX_PLINK_VAL_SCOPE ON BPEL_PLINK_VAL (SCOPE);

DROP INDEX IDX_UNMATCHED_CORR_PROCTYPE;

CREATE INDEX IDX_BPEL_XML_DATA_PIID ON BPEL_XML_DATA(PIID);

DROP INDEX IDX_BPEL_MEX_PROPS_VALUE;
CREATE INDEX IDX_BPEL_MEX_PROPS_VALUE ON BPEL_MEX_PROPS (MEX);

ALTER TABLE vacation ADD substitute VARCHAR2(50);
ALTER TABLE vacation ADD is_active NUMBER;

CREATE TABLE vacation_audit(
	id NUMBER NOT NULL, 
	action_performed VARCHAR2(50), 
	audit_date TIMESTAMP, 
	audit_user_name VARCHAR2(50),
	vacation_id NUMBER,
	updated_from_date DATE,
	updated_to_date DATE,
	updated_description VARCHAR2(255),
	updated_user_name VARCHAR2(50),
	updated_substitute VARCHAR2(50),
	updated_is_active NUMBER,
	PRIMARY KEY (id)
);

...
//Un supported chars & key codes 34 "" 42 * 47 / 58 : 59 ; 60 < 61 = 62 > 63 ? 124 |
var unSupportedKeyCodes = [60,62,63,34,58,42,124,47];
jsx3.lang.Package.definePackage(
  ...
  function(tms) {
    ...
    tms.onGetTaskSuccess = function(event) {
        ...
        // Fix for EDGE-4957 Remove attachment inside GI for PA Task.
        /*if (hasAttachments) {
...
jsx3.lang.Package.definePackage(
  ...
  function(attach) {
    ...
    attach.attachmentNameSubmit = function(e) {
        ...
        else if(unSupportedKeyCodes.indexOf(code)>=0){
           e.preventDefault();
           parent.showInformation("Attachment name should not contain special characters, Unsupported characters are "+unSupportedChars);
           return false; 
        }
	};
	attach.attachmentNamePasteValidate = function(){
        setTimeout(function() {
            if(attach.attachmentNameValid(document.IntalioInternal_AddAttachmentsForm.attachmentName.value)){
                document.IntalioInternal_AddAttachmentsForm.attachmentName.value="";    
                parent.showInformation("Attachment name should not contain special characters, Unsupported characters are "+unSupportedChars);               
            }
        },100);
    };
	attach.fileValidation = function (obj) {
        var fileObj = document.IntalioInternal_AddAttachmentsForm.attachmentFile;
        var ext = fileObj.value.match(/(?:\.([^.]+))?$/)[1];
        if(unSupportedExtensions.indexOf(ext)>=0){
            fileObj.value="";
            window.parent.showInformation("File not supported");
        }
    };
...
jsx3.lang.Package.definePackage(
  ...
  function(util) {
    ...
    util.processTaskType = function(taskType) {
	    ...
	    //Intalio.Internal.Attachments.displayAttachmentsImageBlock(true); Fix for WF-2013 no need to show attachment block as now its handled through dashboard ui.
	}
    ...
    util.closeError = function() {
		...
		// Fix for EDGE-4905
		if ((errorMsg.innerHTML.match(util.getErrorIncompleteField().replace(/({[0-9]})/, "([a-zA-z0-9]*)"))) || (errorMsg.innerHTML.match(util.getErrorIncompleteMatrix())) || (errorMsg.innerHTML.match(util.getErrorUploadingFile().replace(/({[0-9]})/, "([a-zA-z0-9]*)")))) {
			matched = true;

...
jsx3.Class.defineClass("jsx3.gui.RadioButton", jsx3.gui.Block, [jsx3.gui.Form], function(RadioButton, RadioButton_prototype) {
  ...
  RadioButton_prototype.getGroupValue = function() {
    ...
    // if none of the in view radio buttons were selected then iterate over the GI objects
    // EDGE-3195
    var radios = this.getServer().getRootBlock().getDescendantsOfType(jsx3.gui.RadioButton);
    for (var i = 0; i < radios.length; i++) {
      var radio = radios[i];
      if (radio.getGroupName() != this.getGroupName()) continue;
      
      if (radio.getSelected() == RadioButton.SELECTED) return radio.getValue();
    }
    return null;
  };
  ...
  RadioButton_prototype.setGroupValue = function(strValue) {
    ...
    for (var i = 0; i < radios.length; i++) {
      ...
      var checked = radio.getValue(strValue) == strValue;
      radio.setSelected(checked ? RadioButton.SELECTED : RadioButton.UNSELECTED);
    }
  };

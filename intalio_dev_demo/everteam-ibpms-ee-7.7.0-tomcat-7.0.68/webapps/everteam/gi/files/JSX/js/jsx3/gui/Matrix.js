...
// ***********************************************+
  ...
  Matrix_prototype._cleanUpAutoRowSession = function(objEvent,bCleanUpEditSession) {
    ...
    if (jis != null && !jsx3.util.strEmpty(jis.jsxid)) {
      ...
      if (eventRet !== false) {
        // add defaults to empty values for new rows      
        //if (jsx3.gui.Matrix.customSetAutoRowDefaults) {
        //jsx3.gui.Matrix.customSetAutoRowDefaults(this, jis);
        this.customSetAutoRowDefaults(jis);
        //}
  ...
  //THE ORIGINAL FUNCTION WAS PREVIOUSLY PRESENT IN package.js file of com.intalio.ria
  //PACKAGE IN intalioajax-addins AND WAS NAMED AS jsx3.gui.Matrix.customSetAutoRowDefaults
  //MODIFICATIONS MADE TO THE TYPE CHECK MECHANISM
  /**
   * Custom function to fill out default values for empty data cells.
   * This is only invoked on adding a new row to a matrix.
   */
  Matrix_prototype.customSetAutoRowDefaults = function(jis) {
    var columns = this.getDescendantsOfType(jsx3.gui.Matrix.Column, true); // array
    ...
    for (var z = 0; z < columns.length; z++) {
      ...
      if (mask.isType('jsx3.gui.TextBox')) {
        if (val == null || val == undefined) val = "";
      }
      else if (mask.isType('jsx3.gui.Select')) {
        if (val == null || val == undefined) continue;
      }
      else if (mask.isType('jsx3.gui.DatePicker')) {
      ...
      else if (mask.isType('jsx3.gui.TimePicker')) {
        if (val == null || val == undefined || val.trim() == "") continue;
      ...
      else if (mask.isType('jsx3.gui.CheckBox')) {
        if (val == null || val == undefined || val.trim() == "") continue;

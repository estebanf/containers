...
function saveComments(divobj){
  ...
  var user = $('body').find("#userid").text();
...
function addComment(container, type, id, user, comment) {
    ...
    sendAjaxCall(url, "POST", false, true, "html", data, commentErrorCall, function(data1){
...
function addCommentSuccess(container, data, type, id) {
  ...
  $('#'+container.attr('id')).find('#commentsContainerId').empty();
  $('#'+container.attr('id')).find('#commentsContainerId').append(showCommentsHtml(type,container.attr('id')).show());
...
function getCommentsSuccess(container, data,start) {
	...
	if (data.comments.length == 0 && start==0)
		...
		$.each(data.comments, function(i, item) {    
			...
			itemdiv.find( "#name" ).html('<a class="noDecoration" onclick="javascript:showUserProfile(this)" user="'+item.createdBy+'">'+item.createdByName+'</a>');
	...
	applyNiceScroll($('#'+container.attr('id')+'scroll'),300,200,true);
}
...
function commentErrorCall(e) {
	showInformation('Error occurred in getting comment json data '+e.responseText);
	removeLoading('', true);
	return false;
}
...
function send(url,type,data,successCallback) {
	sendAjaxCall(url, type, false, true, "json", data, commentErrorCall, successCallback);
}

/************************************************************************
  ...
  On Windows:
      C:\...> groovy.bat -cp ..\lib\<Database_JAR_path> cleanup_generic.groovy -b 60 -p 8 -s 30 -t 1

  On Linux, Solaris and other Unix-like:
      % ./groovy.sh -cp ../lib/<Database_JAR_path> cleanup_generic.groovy -b 60 -p 8 -s 30 -t 1
...
dburl = "jdbc:sqlserver://localhost:1433;database=bpmsdb"
uname = "root"
passwd = "root"
...
def CleaningDBRecords(def rowsToWork,threadID,tempo)
{
  ...
  try
  {
	...
	for (def k=0; k<LDATA_IDs.size; k=k+1000)
	{
	  def LDATA_last = k+999
	  if(LDATA_last >= LDATA_IDs.size) {
	    LDATA_last = LDATA_IDs.size-1
	  }

	  def IDs = LDATA_IDs[k..LDATA_last]
	  def LDATA_IDParam = "";
	  for(ID in IDs) {
	    LDATA_IDParam += ID +",";
	  }
	...
	for (def k=0; k<EVENT_IDs.size; k=k+1000)
	{
	  def EVENT_last = k+999
	  if(EVENT_last >= EVENT_IDs.size) {
	    EVENT_last = EVENT_IDs.size-1
	  }

	  def IDs = EVENT_IDs[k..EVENT_last]
	  def EVENT_IDParam = "";
	  for(ID in IDs) {
	    EVENT_IDParam += ID +",";
	  }
	  ...
	  if(EVENT_IDParam.size() > 0) {
	    if(LOG_LEVEL <= TRACE) {
	      println "threadID: "+threadID+" EVENT: "+IDs.size()
	    }
